...
class YmlOffer extends \DomElement
{
    ...
    public function __construct(YmlDocument $ymlDocument, $type, $enc)
    {
    ...
    {
        if ($val === false) {
            $newEl = new \DomElement($name);
        } elseif (YmlDocument::haveSpecialChart($val)) {
            $newEl = YmlDocument::createCDATA($this->ymlDocument, $name, $val);
        } else {
            $newEl = new \DomElement($name, $val);
        }
    ...
    /**
     * Сериализация строки от спецсимволов
     */
