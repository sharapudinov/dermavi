...
class AddCancelOrderEmail20190705160023050268 extends BitrixMigration
{
    /**
     * Run the migration.
     *
    ...
    public function up()
    {
        $resultType = EventTypeTable::add([
            'fields' => [
                'LID' => 'ru',
                'EVENT_NAME' => 'ORDER_CANCEL_MANAGER',
                'NAME' => 'Заказ отменен',
        ...
        $resultMessage = EventMessageTable::add([
            'fields' => [
                ...
                'EMAIL_TO' => '#EMAIL_TO#',
                'CC' => '',
                'SUBJECT' => 'Алроса: Отменен заказ',
                'BODY_TYPE' => 'html',
                'MESSAGE' => "<p>Заказ ##ORDER_ID# отменен. Ссылка #ORDER_LINK#</p>"
            ],
        ]);
    ...
    {
        $res = EventTypeTable::getList(["filter" => ["EVENT_NAME" => 'ORDER_CANCEL_MANAGER']]);
        while ($type = $res->fetch()) {
            EventTypeTable::delete($type["ID"]);
        }

        $res = EventMessageTable::getList(["filter" => ["EVENT_NAME" => 'ORDER_CANCEL_MANAGER']]);
        while ($type = $res->fetch()) {
