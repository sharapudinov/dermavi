...
class AddHlCatalogсlarity20181213154014938401 extends BitrixMigration
{
    /**
     * Run the migration.
     *
    ...
    public function up()
    {
        ...
        if (!empty($dict['NAME_RU'])) {
            $result = HighloadBlockLangTable::add([
                "ID" => $highloadBlockId,
                "LID" => "ru",
                "NAME" => 'Справочник цистоты для каталога',
            ]);
        }
    ...
    public function down()
    {
        foreach (static::getListDicts() as $dict) {
            $highloadBlockId = HLblock::getByTableName('catalog_clarity')["ID"];
