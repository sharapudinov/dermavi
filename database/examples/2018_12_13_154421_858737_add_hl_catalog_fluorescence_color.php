...
class AddHlCatalogFluorescenceColor20181213154421858737 extends BitrixMigration
{
    /**
     * Run the migration.
     *
    ...
    public function up()
    {
        ...
        if (!empty($dict['NAME_RU'])) {
            $result = HighloadBlockLangTable::add([
                "ID" => $highloadBlockId,
                "LID" => "ru",
                "NAME" => 'Справочник цветов флуоресценции для каталога',
            ]);
        }
    ...
    public function down()
    {
        foreach (static::getListDicts() as $dict) {
            $highloadBlockId = HLblock::getByTableName('catalog_fluorescence_color')["ID"];
