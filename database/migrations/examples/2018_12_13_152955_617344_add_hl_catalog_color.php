...
class AddHlCatalogColor20181213152955617344 extends BitrixMigration
{
    /**
     * Run the migration.
     *
    ...
    public function up()
    {
        ...
        if (!empty($dict['NAME_RU'])) {
            $result = HighloadBlockLangTable::add([
                "ID" => $highloadBlockId,
                "LID" => "ru",
                "NAME" => 'Справочник цветов для каталога',
            ]);
        }
    ...
    public function down()
    {
        foreach (static::getListDicts() as $dict) {
            $highloadBlockId = HLblock::getByTableName('catalog_color')["ID"];
