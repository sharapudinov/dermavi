...
class AddHlCatalogFluorescence20181213154339113933 extends BitrixMigration
{
    /**
     * Run the migration.
     *
    ...
    public function up()
    {
        ...
        if (!empty($dict['NAME_RU'])) {
            $result = HighloadBlockLangTable::add([
                "ID" => $highloadBlockId,
                "LID" => "ru",
                "NAME" => 'Справочник флуоресценции для каталога',
            ]);
        }
    ...
    public function down()
    {
        foreach (static::getListDicts() as $dict) {
            $highloadBlockId = HLblock::getByTableName('catalog_fluorescence')["ID"];
